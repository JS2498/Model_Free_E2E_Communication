...
def RBF(X, noise_std):
    ...
    X_r = h_r * X[:,:,0] #tf.keras.layers.Multiply()([h_r , X[:,:,0]]) #refer to eq. 11 in the paper
...
class Equalization(tf.keras.layers.Layer):
    def __init__(self):
        ...
        self.h_eq = tf.Variable(tf.zeros(shape = (4,4), dtype = tf.float32), dtype = tf.float32, trainable = False)
...
#%%
#The below function for equalization didn't work out. Hence we built the custom layer
...
train_size = 1000*(2**M) #no. of training examples
...
h_1 = tf.keras.layers.Dense(20, activation = 'tanh', name = "h1")(Y_ip)
...
rx_enmsg = tf.keras.layers.Lambda(lambda x: RBF(x, noise_std))(encoded_msg) # channel_layer(encoded_msg) #received encoded message
...
# AE_AL.fit(train_messages,train_messages,batch_size=b_size,epochs = n_epoch, validation_split= 0.1 , verbose = 1)
